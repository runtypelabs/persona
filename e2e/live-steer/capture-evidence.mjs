...
if (!['before', 'after'].includes(label) || !source || !output) {
  throw new Error('Usage: node e2e/live-steer/capture-evidence.mjs before|after <source-repo> <output-directory>');
}
...
const server = spawn(process.execPath, ['e2e/live-steer/server.mjs'], {
  env: { ...process.env, PERSONA_STEER_DIST: resolve(source, 'packages/widget/dist') },
...
try {
  ...
  for (const viewport of [{ width: 1100, height: 900 }, { width: 390, height: 844 }]) {
    ...
    await input.fill('[[mock:tool name=mcp_custom_steer_fixture_slow_tool args={"delayMs":8000}]]');
    await input.press('Enter');
    await page.waitForFunction(() => window.steerController.getMessages().some(message => message.toolCall?.name?.includes('slow_tool') && message.toolCall.status !== 'complete'));
