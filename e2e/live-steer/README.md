# Real Core live-steer browser test
...
2. From Core, provision a fixture with the exported
   `.github/smoke-tests/lib/client-steer-fixture.ts` helper:
   `createClientSteerFixture(apiUrl, apiKey, 'http://127.0.0.1:4318')`.
...
3. In Persona, run:
   ...
   ```sh
   pnpm build:widget
   PERSONA_STEER_FIXTURE=/absolute/path/to/fixture.json \
     pnpm exec playwright test --config e2e/live-steer/playwright.config.ts
   ```

4. In a `finally` block, delete the fixture's synthetic conversation records and
   call `cleanupClientSteerFixture(fixture, apiKey)`. Delete the temporary JSON
...
The Core `client-session-steer` registered smoke independently exercises API
admission, idempotency conflicts, receipt privacy, and persisted history. Both
suites should pass before publishing changes to the steer contract.
