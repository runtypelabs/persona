...
**WebMCP page tools:** `webmcp.enabled` snapshots tools from `document.modelContext`, sends them as `clientTools[]`, executes returned `webmcp:<name>` calls in the browser with approval gating, and resumes via `${apiUrl}/resume`. `contextProviders` run on both agent and flow/proxy paths. Because `document.modelContext` is page-global, any same-origin script can register a tool, so the bridge hardens against tool injection: it sanitizes tool name/description (`utils/webmcp-sanitize.ts`), tags tool output `untrustedContentHint`, surfaces the registering `pageOrigin` in the confirm gate, and flags a tool whose contract changed since the dispatch snapshot (or was never offered) as `suspicious` (which also bypasses `autoApprove`). See `packages/widget/WEBMCP-SECURITY.md` for the full threat model.
