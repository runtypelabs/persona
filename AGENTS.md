...
**Does NOT need a changeset:** Changes to `examples/`, `docs/`, CI config, tooling, or `AGENTS.md`.
...
**CORS / `NODE_ENV`:** The canonical `runtype-hono-proxy` example always supplies an origin allowlist; when `ALLOWED_ORIGINS` is unset it defaults to the local showcase origins. The bare published `createChatProxyApp()` is permissive when `allowedOrigins` is omitted or empty. With an allowlist, non-matching origins are reflected only when `NODE_ENV` is **exactly** `"development"`, when `VERCEL_ENV` is `"preview"`, or when they match `previewOriginPattern`; an unset `NODE_ENV` is treated as production. The root `pnpm dev` script sets `NODE_ENV=development` automatically. If you start the example proxy directly (e.g. `pnpm dev:runtype-hono-proxy`), set it yourself or configure it in your `.env` file.

**Server-pinned agents:** `createChatProxyApp()` supports `agentConfig` and `agentId` for agent-shaped routes with server-side security. These options are mutually exclusive with `flowId` / `flowConfig`; the proxy builds the upstream agent payload itself and ignores any client-supplied `agent`. A client-supplied `agent` on a non-server-agent route is rejected with HTTP 400. Pin agents server-side, or use a separately authenticated and authorized backend when browser-selected agent definitions are intentional.
