...
Opt in to non-interrupting sends with `composer.streamingSubmitBehavior: "join"` for native durable client-token chat. Sends preserve active tools and questions; a separate Stop button cancels the run. See [Live input joining](./docs/live-input-joining.md) for requirements, delivery states, and retry behavior.
