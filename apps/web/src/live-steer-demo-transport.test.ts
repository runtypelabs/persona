...
import {
  ...
  LIVE_STEER_DEMO_ORIGIN as origin,
} from "./live-steer-demo-transport";
let demo: ReturnType<typeof createLiveSteerDemoTransport>;
afterEach(() => {
  demo?.dispose();
  vi.useRealTimers();
});
...
const status = (execution: string, delivery: string) =>
  demo
    .fetch(
      `${origin}/v1/client/conversations/record_live_steer_demo/executions/${execution}/deliveries/${delivery}`,
    )
    .then((response) => response.json());
describe("live steer teaching transport", () => {
  it("keeps one execution and settles every steered input in its streamed response", async () => {
    vi.useFakeTimers();
    demo = createLiveSteerDemoTransport({ delayMs: 8000 });
    const first = await send("first");
    const output = first.text();
    const steered = await send("second");
    expect(steered.status).toBe(202);
    const receipt = await steered.json();
  ...
  it("deduplicates a lost acknowledgement and rejects changed content", async () => {
    demo = createLiveSteerDemoTransport();
    await send("first");
  ...
  it("stops the host, marks pending input not applied, and admits a new execution", async () => {
    demo = createLiveSteerDemoTransport();
    await send("first");
    const steered = await (await send("second")).json();
    const cancelled = await demo.fetch(
      `${origin}/v1/client/conversations/record_live_steer_demo/executions/${steered.executionId}/cancel`,
      { method: "POST" },
    );
    expect(cancelled.status).toBe(202);
    expect((await status(steered.executionId, steered.deliveryId)).status).toBe(
      "not_applied",
    );
