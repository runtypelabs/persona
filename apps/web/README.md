...
- **Paint page**: `http://localhost:5173/webmcp-paint.html` (proxy mode: agent defined in code as `WEBMCP_PAINT_FLOW`, mounted at `/api/chat/dispatch-paint`)
  ...
  - jspaint ships as a pinned **git dependency** (`runtypelabs/jspaint` fork) served at a content-versioned `/jspaint-<hash>/` path by the `serveJsPaint` Vite plugin (so every jspaint asset ships with `Cache-Control: immutable`; the hash covers the pinned SHA and the bridge module): nothing is vendored into this repo. Same-origin serving is load-bearing: `src/webmcp-paint/jspaint-host.ts` injects `public/jspaint-bridge.mjs` into the iframe (re-injected on every load : jspaint replaces its document during session bootstrap), which imports jspaint's own ES modules and reaches its lexical-global app state, something the iframe's CSP forbids doing via eval or inline scripts
...
Open `/live-steer-demo.html` (also listed under Patterns and in the command palette).
...
For a real native durable agent, set `VITE_LIVE_STEER_CLIENT_TOKEN` (browser-safe
client token, **never an API key**) and `VITE_LIVE_STEER_API_URL` in
