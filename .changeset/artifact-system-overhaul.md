...
- Markdown artifacts carrying optional `file` metadata (path + mimeType) can now preview HTML/SVG in a sandboxed `<iframe srcdoc>` (opaque origin, `allow-scripts` by default, never `allow-same-origin`), render markdown through the usual pipeline, or show source. Cards show the file type and basename, and downloads use the real filename, MIME type, and unfenced content. Configure via `features.artifacts.filePreview` (`enabled`, `iframeSandbox`). Fully additive: artifacts without `file` behave exactly as before.
...
- Artifact pane tabs no longer overflow the panel: the tab strip scrolls horizontally, tabs stay on one line and truncate with an ellipsis, file tabs are labelled by basename with the full path in a tooltip, and the selected tab is scrolled into view on selection change.
...
- CDN build: artifacts upserted immediately after init no longer render as escaped plain text; the shared preview body re-renders once the lazy markdown-parsers chunk loads and no longer double-escapes the fallback. `loadMarkdownParsers` is exported from the public API so hosts can await parser readiness before injecting content.
